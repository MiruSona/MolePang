...
public class EnemyManager : SingleTon<EnemyManager> {
    ...
    //스테이지
    private int stage = 1;
    private int stage_more_num = 3;         //5스테이지 마다 더 많은 수 등장
    private int stage_bomb = 3;             //폭탄 등장
    private int stage_mole_hat = 9;        //모자쓴 두더지 등장
    private int stage_dec_time = 12;        //시간 줄어듬
    ...
    public void GameStart(int _stage)
    {
        ...
        //적 숫자 조절
        if (stage == stage_bomb)
            total_num = start_num + 1;
        else if (stage >= stage_mole_hat)
            total_num = total_max;
        else
            total_num = start_num;
